...
Recently, \citep{Wogan_2023} use photochemical models of post-impact atmospheres to show that impacts significantly smaller than Moneta should efficiently produce prebiotic molecules. In an optimistic modeling case, a $> 4 \times 10^{20}$ kg impact produces atmosphere that generates copious prebiotic molecules, while in their most pessimistic scenario a $> 5 \times 10^{21}$ kg impactor is required. It is therefore possible that an impactor much smaller than Moneta caused the origin of life, and that the \citet{Benner_2020} estimated timing for biopoesis, based on Moneta starting life, could be inaccurate.

Here, we use the \citep{Wogan_2023} results, along with Monte-Carlo simulations of Earth's impact history to make an alternative estimate for when life most likely emerged during an RNA-first process.
